...
function debitar(valor: number): void {
    if (valor <= 0) {
        throw new Error("0 valor a debitar deve ser maior que zero.");
    }
    if (valor > saldo) {
...
const Conta = {
    ...
    getGruposTransacoes(): GrupoTransacao[] {
        ...
        const tramsacoesOrdenadas: Transacao[] = listaTransacoes.sort((t1, t2) => t2.data.getTime() - t1.data.getTime());
        let labelAtualGrupoTrasacao: string = "";

        for (let transacao of tramsacoesOrdenadas) {
